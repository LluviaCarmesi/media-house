...
<div class="pagerContainer">
    {#if currentPage - 1 > 0}
    <a on:click={() => openPage(currentPage - 1)} href={`${pageQueryString}=${currentPage - 1}`}>&lt</a>
    {/if}
    {#each pagesToShow as pageToShow}
        {#if pageToShow === currentPage}
            <a
                class="currentPage" on:click={() => openPage(pageToShow)}
                href={`${pageQueryString}=${pageToShow}`}
            >{pageToShow}</a>
        {:else}
            <a
    ...
    {#if currentPage + 1 <= maxPages}
        <a on:click={() => openPage(currentPage + 1)} href={`${pageQueryString}=${currentPage + 1}`}>&gt</a>
    {/if}
</div>